...
class roles extends Component {
    ...
    renderRows(indice) {
        ...
        try {
            ...
            let header = Object.keys(this.state.roles[0]);
            return header.map((key, index) => {
                var value = (this.isDate(this.state.roles[indice][key]) ? this.parseMoment(this.state.roles[indice][key]) : this.state.roles[indice][key]);
                var path = "/actualizarRol/" + value;
                if (index === 0) {
                    return <td key={index}>
